...
ROOT_ID = "r-7w8p"
SOURCE_ASSUMED_ROLE = "OrganizationAccountAccessRole" 
NEW_RESOLVER_RULE = "rslvr-rr-b333b2fa07e04cddb"
SOURCE_MASTER = "741252614647"
TARGET_MASTER = "662627786878"

class Account():
    def __init__(
            self, account_id, logger, master_obj=None, org_root=ROOT_ID, role_name=SOURCE_ASSUMED_ROLE,
            region="us-west-2"):
        self.logger = logger
    ...
    def create_management_role(self):
        ...
        try:
            ...
            self.logger.info(
                f"AWSControlTowerExecution Role already exists " 
                f"in account {self.accountid}."
            )    
            # Insert Code to append TARGET_MASTER to 
            # trustPolicy['Statement'][0]['Principal']['AWS"] 
...
class Master():
    def __init__(self, accountId, logger):
        ...
        if accountId == SOURCE_MASTER:
            self.session = boto3.session.Session(
                                profile_name="master",
                                region_name="us-west-2"
                            )
            self.rootId = "r-7w8p"
            self.executionRole = SOURCE_ASSUMED_ROLE
            self.org = self.session.client("organizations")
            self.sts = self.session.client("sts")
            self.guardduty = self.session.client("guardduty")
            self.detectorId = self.guardduty.list_detectors()['DetectorIds'][0]
            self.securityhub = self.session.client("securityhub")
        elif accountId == TARGET_MASTER:
            self.session = boto3.session.Session(
...
if __name__ == "__main__":
    ...
    legacy_master = Master(SOURCE_MASTER, logger)
    new_master = Master(TARGET_MASTER, logger)
    ...
    for account in accounts:
        ...
        # Add try/except to catch if target OU doesn't exist, and handle where to move if it doesn't.
        # new_master.find_ou(identifier=accnt.ou['Name'])
