...
permission_sets = []
response = sso.list_permission_sets(InstanceArn=sso_instances[0]['InstanceArn'], MaxResults=100)
permission_sets.extend(response['PermissionSets'])
while 'NextToken' in response:
    response = sso.list_permission_sets(InstanceArn=sso_instances[0]['InstanceArn'], MaxResults=100, NextToken=response['NextToken'])
    permission_sets.extend(response['PermissionSets'])
