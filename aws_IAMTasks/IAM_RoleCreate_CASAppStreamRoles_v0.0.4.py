...
def create_statement(actions, effect, resources = None, principal=None, conditions=None):
    statement = {
        "Effect": effect,
        "Action": []
    }
...
def extract_substring(item):
    if item.startswith("CAS_FP_"):
        return item[7:]  # Length of "CAS_FP_" is 7
    elif item.startswith("CAS_"):
        return item[4:]  # Length of "CAS_" is 4
    return item  # Return the original item if neither prefix is found    
...
for location in location_list: 
    ...
    location_partition = extract_substring(location)
    location_partition = location_partition.lower()
    resource_list = [
        generate_resource('ssm','us-west-2','827354439487',
            f'parameter/locations/{location_partition}/{parameter}'
    ...
    if not TEST: 
        try:
            logger.info(f"Creating Role {iam_role_params['RoleName']}")
            iam_role_details = iam.create_role(**iam_role_params) 
            logger.info(f"Role {iam_role_details['Role']['RoleName']} created.")
            waiter.wait(RoleName=iam_role_details['Role']['RoleName'])  
            logger.info(f"Role {iam_role_details['Role']['RoleName']} now exists.")
    ...
    if not TEST:
        logger.info(f"Putting Role Policy {iam_policy_params['PolicyName']} on Role {iam_policy_params['RoleName']}")
