...
for accountId in accounts:
    ...
    elif ((itOpsArn not in permission_sets) and (accountId in target_accounts)):
        # continue
        print(f"{accountId} does not contains the TMK_ITOps_SystemInfrastructure Permission Set")
        updated_accounts['Accounts'].append(accountId)
        print(f"Delegating TMK_ITOps_SystemInfrastructure access to group for Account: {accountId}")
        assignment_request = sso.create_account_assignment(
            InstanceArn=instanceArn,
            TargetId=accountId,
            TargetType='AWS_ACCOUNT',
