        ...
        for loadBalancer in elb_list:
            ...
            for group in targetGroups:        
                ...
                for target in tarHealthDescriptions:
                    ...
                    if targetDict not in lbDetail['Instances']:
                        lbDetail['Instances'].append(targetDict)
...
filename = (f"LoadBalancer_Instance_Mapping.json")
excelfilename = ("LoadBalancer_Instance_Mapping.xlsx")
export_path = (
...
df_lbinfo = pd.DataFrame.from_dict(pub_lbs, orient='index')
completeFilePath = os.path.join(export_path, filename)
excelfilepath = os.path.join(export_path, excelfilename)    
df_lbinfo.to_excel(excelfilepath)
with open(completeFilePath, "w") as f:
    f.write(
        json.dumps(pub_lbs, sort_keys=True, indent=4, default=str
        )
    )
