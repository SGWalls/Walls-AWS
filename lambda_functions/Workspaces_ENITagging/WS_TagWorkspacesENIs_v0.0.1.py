...
os.environ['AWS_PROFILE'] = 'user_svcs'
...
def manual_trigger(interface_id):
    event = {
        "detail": {
            "responseElements": {
                "networkInterface": {
...
ec2_usrclient = boto3.resource('ec2')
eni_list = ec2_usrclient.network_interfaces.all()
for eni in eni_list:
    manual_trigger(eni.id)