...
def check_account(account, role_name, master_session):
    ...
    if account['Status'] == 'ACTIVE':
        try:
            ...
            results = check_rds_audit_logging(assumed_session)
            log_disabled.extend([db for db in results.keys() if results[db]['audit_enabled'] == False ])
            return log_disabled
        except Exception as e:
...
if __name__ == "__main__":
    ...
    final_result = {}
    # log_disabled = []
    session = get_session(profile_name='ct_master')
    ...
    for account in get_member_accounts(session.client('organizations')):
        log_result = check_account(account,"AWSControlTowerExecution",session)
        if log_result:
            final_result[account['Id']] = log_result
    print("RDS databases without audit logging enabled: ", final_result)
