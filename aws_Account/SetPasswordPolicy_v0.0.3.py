...
class Account():
    ...
    # def check_password_policy(self, password_policy):
    #     iam = self.client_config('iam')
    #     try:
    #         response = iam.get_account_password_policy()
    #         return response['PasswordPolicy'] == password_policy
    #     except Exception as e:
    #         self.logger.error(f"Error checking password policy for account {self.account_id}: {e}")

    def check_password_policy(self, new_policy):
        """
        Check if the current account password policy matches the new policy.
        
        :param new_policy: A dictionary containing the new password policy settings
        ...
        try:
            ...
            # Compare the current policy with the new policy
            for key, value in new_policy.items():
                if key in current_policy and current_policy[key] != value:
                    return False
            ...
            if e.response['Error']['Code'] == 'NoSuchEntity':
                # No password policy exists, so it doesn't match the new policy
                self.logger.error(f"Error checking password policy for account {self.account_id}: {e}")
                return False
            else:
    ...
    def update_password_policy(self, password_policy):
        iam = self.client_config('iam')
        try:
            if not self.check_password_policy(password_policy):
                ...
                self.logger.info(f"Password policy is already set for account {self.account_id}")
        except Exception as e:
...
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = [executor.submit(process_account, account) for account in account_list]
