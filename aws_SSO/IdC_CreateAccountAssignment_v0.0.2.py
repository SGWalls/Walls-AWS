...
class Assign():
    ...
    def get_permission_set_detail(self, permission_set_list,permission_set_name):
...
if __name__ == "__main__":
    ...
    dir_detail = permission_action.get_directory_details()
    instance_arn = sso_admin.list_instances(MaxResults=1)['Instances'][0]['InstanceArn']
    grp_detail = permission_action.get_group_details(permission_action.group_list, group_name)
    if not grp_detail:
        raise Exception('Group not found')
    permission_set_detail = permission_action.get_permission_set_detail(permission_action.perm_set_list, permission_set_name)
    account_list = permission_action.get_all_accounts()
    print(grp_detail)
    print(permission_set_detail)
    for account in account_list:
        if account['Id'] in ACCOUNT_EXCEPTIONS:
            print(f'Skipping account {account["Name"]} : {account["Id"]}')
            continue
