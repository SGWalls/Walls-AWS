...
TARGET_ACTION        = ['iam:CreatePolicy', 'iam:Create*', 'iam:*Policy', 'iam:*']
ALLOW_RESOURCE       = ['*', 'arn:aws:iam::*:policy/Boundary_*', 'arn:aws:iam::*:policy/*', 'arn:aws:iam::*:*', 'arn:aws:iam:*']
...
def action_matches(action, targets):
    ...
    for target in targets:
        ...
        if action in ('*', target):
            return True
        if fnmatch.fnmatch(target, action):  # policy action wildcard covers target (e.g. iam:* covers iam:createpolicy)
            return True
        if fnmatch.fnmatch(action, target):  # target pattern matches policy action (e.g. iam:create* matches iam:createpolicy)
            return True
    return False


def resource_matches(resource, targets):
    """Returns True if the policy resource covers any of the target resources."""
    ...
    for target in targets:
        ...
        if resource in ('*', target):
            return True
        if fnmatch.fnmatch(target, resource):  # policy resource wildcard covers target
            return True
        if fnmatch.fnmatch(resource, target):  # target pattern matches policy resource
            return True
    return False


def has_allow_create_policy_on_star(statements):
    """Returns True if any Allow statement grants a TARGET_ACTION on any ALLOW_RESOURCE."""
    for stmt in statements:
        ...
        if isinstance(resources, str):
            resources = [resources]
        if any(action_matches(a, TARGET_ACTION) for a in actions) and \
           any(resource_matches(r, ALLOW_RESOURCE) for r in resources):
            return True
    return False
...
def has_deny_create_policy_on_boundary_prefix(statements):
    ...
    for stmt in statements:
        ...
        if any(resource_matches(r, REQUIRED_DENY_RESOURCE) for r in resources):
            return True
    return False
